...
#define MODID 123
#define NAMELONG L"AFSIO Module 8.0.3.0"
...
KEXPORT DWORD afsioAfterGetBinBufferSize(GET_BIN_SIZE_STRUCT* gbss, DWORD orgSize)
{
    ...
    TRACE2N(L"afsId=%d, binId=%d", gbss->afsId, gbss->binId);
    ...
    if (hfile && fsize)
    {
        ...
        TRACE4N(L"afsId=%d, binId=%d, orgSize=%0x, newSize=%0x", gbss->afsId, gbss->binId, orgSize, fsize);
    }
    return result;
}
...
KEXPORT void afsioAtGetSize(DWORD afsId, DWORD binId, DWORD* pSizeBytes, DWORD* pSizePages)
{
    TRACE4N(L"afsAtGetSize: afsId=%d, binId=%d, sizeBytes=%08x, sizePages=%08x",
